...
def find_password(request):   ## 아직 구현중입니다.
    if request.method == "POST":
        ...
        try:
            ...
            if request.POST.get('new_password'):
                ...
                if form.is_valid():
                    form.save()
                    # 변경된 비밀번호를 로그인한 사용자에게 적용하기 위해 로그아웃하고 다시 로그인합니다.
                    logout(request)
                    ...
                    return render(request, 'users/find_password.html', {'password_found': True, 'password_changed': True})
                else:
